...
manhattan(qqdf,logp = FALSE, ylim=c(0,1), ylab="Fst")
...
outliers <- P1$LocusName[which(P1$OutlierFlag ==TRUE)] #which of the SNPs are outliers? Note that this only returns the index
length(outliers)

```

Now we can make a manhattan plot:
...
path_to_file <- "~/my_materials/population_structure/variants_NoLD.bed"
...
# keep to left of traight line

# read in individual ids, in same order as vcf/plink file
population <- read.table("~/shared_materials/tutorial_files/pops.txt",
                          header=F)
plot(x, option = "scores", pop = population$V1)

plot(x, option = "scores", i = 3, j = 4, pop = population$V1)

plot(x , option = "manhattan")


# thin for ld
res <- pcadapt(input = filename, K = 20, LD.clumping = list(size = 200, thr = 0.1))
plot(x, option = "scores", pop = population$V1)
...
# make a new dataframe that fits the qqman requirements:
##
infs <- read.table("~/my_materials/population_structure/variants_NoLD.bim")

qqdf <- data.frame(SNP = infs$V2,
                   CHR = infs$V1,
                   BP = infs$V4,
                   P = x$pvalues
)

sigloci <- qqdf$SNP[outliers]

# make the manhattan plot
library(qqman)
manhattan(qqdf,logp = TRUE, ylab="Fst", highlight = sigloci, ylim=c(0,80))


# can also filter by LD even more.
