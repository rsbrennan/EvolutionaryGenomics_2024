...
Add various combinations of the options `l`, `r`, `t`, `h` to your ls command. Note that you only need one `-` so you can enter `ls -lr` for example. 
...
Note that the last examples combine multiple options but only use one dash. This is a very common way of specifying multiple command-line options. Trial and error is a good way to figure out what things do, but there are easier, faster, and likely safer ways. It's time to learn about Unix documentation....
...
To do this use the [rmdir][] command, this will only remove empty directories so it is quite safe to use. If you want to know more about this command (or any Unix command), then remember that you can just look at its man page.

<br>

<div class="text-box">

Move into `~/Learning_unix/Outer_directory/` and remove `Inner_directory`.

Verify that you have removed this directory. 

Remove `Outer_directory`
...
Alternatively, you could have moved to `~Learning_unix` first and then copied `file3` using `cp ~/file3 .`. 

Delete `file3` and try this second approach now.
...
## 22: Counting characters and lines in a file ##
...
## 23: Matching lines in files with grep ##
...
Make sure you can run the above commands.
