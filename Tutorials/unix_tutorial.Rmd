...
From time to time this documentation will contain [web links][] to pages that will help you find out more about certain Unix commands. Usually, the _first_ mention of a command or function will be a hyperlink to Wikipedia. 
...
In the following documentation, we will also assume that the logged in user has a username 'jovyan' and the home directory is located at `/home/jovyan`.
...
1. You may probably see different output to what is shown here, it depends on your computer setup. Don't worry about that for now. 
2. The `jovyan@cloudlab:~$` text that you see is the Unix [command prompt][]. In this case, it contains a user name ('jovyan') and the name of the current directory ('~', more on that later). Note that the command prompt might not look the same on different Unix systems. In this case, the `$` sign marks the end of the prompt. 
...
jovyan@cloudlab:~$ ls /usr
bin  etc  games  include  lib  lib32  lib64  libexec  libx32  local  sbin  share  src
```
...
```bash
jovyan@cloudlab:~$ mkdir Learning_unix
jovyan@cloudlab:~$ ls
Learning_unix  my_materials  shared_materials
```
...
```bash
jovyan@cloudlab:~/Learning_unix$ mkdir Outer_directory
jovyan@cloudlab:~/Learning_unix$ cd Outer_directory
jovyan@cloudlab:~/Learning_unix/Outer_directory$

jovyan@cloudlab:~/Learning_unix/Outer_directory$ mkdir Inner_directory
jovyan@cloudlab:~/Learning_unix/Outer_directory$ cd Inner_directory/
jovyan@cloudlab:~/Learning_unix/Outer_directory/Inner_directory$
```
...
```bash
jovyan@cloudlab:/home$ cd /
jovyan@cloudlab:/$ cd home/
jovyan@cloudlab:/home$ cd jovyan/
```
...
```bash
jovyan@cloudlab:~$ cd Learning_unix/
jovyan@cloudlab:~/Learning_unix$ cd ..
jovyan@cloudlab:~$ cd ..
jovyan@cloudlab:/home$
```
...
```bash
jovyan@cloudlab:~/Learning_unix$ cd ~/Learning_unix/Outer_directory/
jovyan@cloudlab:~/Learning_unix/Outer_directory$ ls ../../
...
```bash
jovyan@cloudlab:~/Learning_unix$ ls -l /home
total 0
drwxr-x--- 1 jovyan jovyan 117 Mar 28 12:08 jovyan
```
...
```bash
jovyan@cloudlab:~$ cd ~/Learning_unix/Outer_directory/
jovyan@cloudlab:~/Learning_unix/Outer_directory$ rmdir Inner_directory/
jovyan@cloudlab:~/Learning_unix/Outer_directory$ cd ..
jovyan@cloudlab:~/Learning_unix$ rmdir Outer_directory/
jovyan@cloudlab:~/Learning_unix$ ls
jovyan@cloudlab:~/Learning_unix$
```
...
```bash
jovyan@cloudlab:~$ cd Learning_unix/
jovyan@cloudlab:~/Learning_unix$ touch heaven.txt
jovyan@cloudlab:~/Learning_unix$ touch earth.txt
jovyan@cloudlab:~/Learning_unix$ ls
...
```bash
jovyan@cloudlab:~/Learning_unix$ mkdir Temp
jovyan@cloudlab:~/Learning_unix$ mv heaven.txt Temp/
jovyan@cloudlab:~/Learning_unix$ mv earth.txt Temp/
jovyan@cloudlab:~/Learning_unix$ ls
Temp
...
```bash
jovyan@cloudlab:~/Learning_unix$ touch rags
jovyan@cloudlab:~/Learning_unix$ ls
rags  Temp
jovyan@cloudlab:~/Learning_unix$ mv rags Temp/riches
jovyan@cloudlab:~/Learning_unix$ ls Temp/
...
```bash
jovyan@cloudlab:~/Learning_unix$ mv Temp/riches Temp/rags
```
...
```bash
jovyan@cloudlab:~/Learning_unix$ mv Temp/riches Temp/rags
jovyan@cloudlab:~/Learning_unix$ mkdir Temp2
jovyan@cloudlab:~/Learning_unix$ mv Temp2 Temp
jovyan@cloudlab:~/Learning_unix$ ls Temp/
...
```bash
jovyan@cloudlab:~/Learning_unix$ cd Temp
jovyan@cloudlab:~/Learning_unix/Temp$ ls
earth.txt  heaven.txt  rags  Temp2
jovyan@cloudlab:~/Learning_unix/Temp$ rm -i earth.txt heaven.txt rags
...
```bash
jovyan@cloudlab:~/Learning_unix$ touch ~/file3
jovyan@cloudlab:~/Learning_unix$ ls ~
command_line_course  file3  Learning_unix  linux_bootcamp
jovyan@cloudlab:~/Learning_unix$ cp ~/file3 .
jovyan@cloudlab:~/Learning_unix$ ls
file1  file2  file3
```
...
```bash
jovyan@cloudlab:~/Learning_unix$ echo "Call me Ishmael."
Call me Ishmael.
jovyan@cloudlab:~/Learning_unix$ echo "Call me Ishmael." > opening_lines.txt
jovyan@cloudlab:~/Learning_unix$ ls
opening_lines.txt
jovyan@cloudlab:~/Learning_unix$ less opening_lines.txt
```
...
```bash
jovyan@cloudlab:~/Learning_unix$ echo "The primroses were over." >> opening_lines.txt
jovyan@cloudlab:~/Learning_unix$ cat opening_lines.txt
Call me Ishmael.
...
## 24: Viewing parts of files with head and tail  ##

In some cases we might be interested in checking if their format is correct or if the file contains the expected information throughout its entirety. For this, we can use the [head][] command to print the first 10 lines of a file. Similarly, the [tail][] command prints the last 10 lines of the file. These commands are particularly useful when dealing with files with millions of lines or when viewing multiple files simultaneously.

You would run the commands as follows:

```bash 
jovyan@cloudlab:~/Learning_unix$ head opening_lines.txt
jovyan@cloudlab:~/Learning_unix$ tail opening_lines.txt
...
## 25: Counting characters and lines in a file ##

```bash
jovyan@cloudlab:~/Learning_unix$ ls
opening_lines.txt

jovyan@cloudlab:~/Learning_unix$ ls -l
total 4
-rw-rw-r-- 1 ubuntu ubuntu 42 Jun 15 04:13 opening_lines.txt

jovyan@cloudlab:~/Learning_unix$ wc opening_lines.txt
 2  7 42 opening_lines.txt

jovyan@cloudlab:~/Learning_unix$ wc -l opening_lines.txt
2 opening_lines.txt
```
...
```bash
jovyan@cloudlab:~/Learning_unix$  echo $HOME
/home/jovyan
jovyan@cloudlab:~/Learning_unix$ echo $PATH
/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin:/usr/local/bin/FastQC:/usr/local/bin/admixture_linux-1.3.0:/usr/local/bin/TrimGalore-0.6.10:/usr/local/bin/plink_linux_x86_64_20230116
```
...
Open the launcher tab and make a new textfile to add the following lines to your file:
...
```bash
jovyan@cloudlab:~/Learning_unix$ grep was opening_lines.txt | wc -c
316
